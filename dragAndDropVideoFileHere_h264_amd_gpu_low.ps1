...
$script = {
    ...
    $cmd = ".\ffmpeg.exe -i `"$Arg1`" -vcodec h264_amf -rc cqp -qp_i 29 -qp_p 30 -r 24 -vbaq true -high_motion_quality_boost_enable true -map `"0:v`" $audioArgs -b:a 64k -map `"[0:a]`" `"$Arg3`""
    echo "$cmd"
