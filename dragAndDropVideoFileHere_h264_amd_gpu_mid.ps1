...
$script = {
    ...
    $cmd = ".\ffmpeg.exe -i `"$Arg1`" -vcodec h264_amf -rc cqp -qp_i 19 -qp_p 20 -r 30 -vbaq true -high_motion_quality_boost_enable true -quality 0 -map `"0:v`" $audioArgs -map `"[0:a]`" `"$Arg3`""
    echo "$cmd"
